...
{
  if (Wire.available() >= 6) {
    // Read incoming data from the ESP32
    leftMotor_speed = Wire.read() << 8 | Wire.read();
    rightMotor_speed = Wire.read() << 8 | Wire.read();
    servoAngle = Wire.read() << 8 | Wire.read();

    // Limit the values to the acceptable range
